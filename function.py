...
def process_cropped_image(cropped_image_path, reader):
    text = process_image(cropped_image_path, reader)
    return cropped_image_path, text


def process_img(image_path, index):
    reader = initialize_reader()
    image = cv2.imread(image_path)
    if image is None:
        return [(image_path, f"Failed to load image: {image_path}\n", index)]
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for region in regions:
            ...
            futures.append(executor.submit(
                process_cropped_image, cropped_image_path, reader))

        for future in futures:
            cropped_image_path, text = future.result()
            results.append((cropped_image_path, text, index))
...
class ImageProcessorApp:
    def __init__(self, root):
        ...
        self.add_button = tk.Button(
            self.button_frame, text="Add Images", command=self.add_images)
        self.add_button.pack(side=tk.LEFT, padx=5)

        self.remove_button = tk.Button(
            self.button_frame, text="Remove Selected", command=self.remove_images)
        self.remove_button.pack(side=tk.LEFT, padx=5)

        self.process_button = tk.Button(
            self.button_frame, text="Process Images", command=self.process_images)
        self.process_button.pack(side=tk.LEFT, padx=5)

        self.save_button = tk.Button(
            root, text="Save Output", command=self.save_output)
        ...
        self.text_outputs = [
            tk.Text(self.text_output_frame, width=50, height=10) for _ in range(6)]
    ...
    def add_images(self):
        ...
        if len(file_paths) > 6:
            messagebox.showwarning(
                "Warning", "Please select up to 6 images only.")
            return
    ...
    def process_images(self):
        ...
        if len(self.selected_images) > 6:
            messagebox.showwarning(
                "Warning", "Only the first 6 images will be processed.")
            selected_to_process = self.selected_images[:6]
        else:
            selected_to_process = self.selected_images

        pool = multiprocessing.Pool(processes=6)
        results = [pool.apply_async(process_img, args=(image_path, i))
                   for i, image_path in enumerate(selected_to_process)]

        pool.close()
        pool.join()

        output_results = [result.get() for result in results]
        for result in output_results:
            for image_path, text, index in result:
                self.root.after(0, self.insert_text, image_path, text, index)
    ...
    def save_output(self):
        output_texts = [text_output.get("1.0", tk.END)
                        for text_output in self.text_outputs]
